...
final class Animation {
    ...
    func play(x: CGFloat, y: CGFloat, type: AnimationType, referenceView: UIView){
        ...
        case .hit:
            ...
            animate(x: x, y: y, type: .hit, animations: animations, speed: 3, referenceView: referenceView)
        case .miss:
            let animations = ["animation3"]
            animate(x: x, y: y, type: .miss, animations: animations, speed: 3, referenceView: referenceView)
        case .shot:
            let animations = ["animation1", "animation2", "animation3"]
            animate(x: x, y: y, type: .shot, animations: animations, speed: 5, referenceView: referenceView)
            
        }
        
    }
    
    private func animate(x: CGFloat, y: CGFloat, type: AnimationType, animations:[String], speed: CGFloat, referenceView: UIView){
    ...
    private func createAnimationView(x: CGFloat, y: CGFloat, name: String , type: AnimationType, speed: CGFloat) -> LottieAnimationView {
        ...
        switch type {
            case .hit, .shot:
                let point = CGPoint(x: animationViewX, y: animationViewY)
                ...
                case .right:
                    animationViewX = CGFloat(x - animationViewWidth)
                    animationViewY = CGFloat(y + animationViewWidth / 2)
                }
